...
function BuyProduct1() {
  ...
  const [product, setProduct] = useState(
    {
      src: anh1,
      name: 'Beautiful modern wooden wardrobe with sliding doors – TQAVP09',
      price: 5000
  ...
  return (
    ...
    <div className='buyproduct1'>
      {!isShowCart && (<>
        <div className='btn-neo'><Header quantity={Cart.length} setShowCart={setShowCart} /></div>
          ...
          <div className='buy'>
            ...
            <div className='btn-buy'>
              {Cart.indexOf(product) !== -1 ? (
                <div className='datontai'>
                  <span >
                    Add To Cart success
                  </span>
                </div>
              ) : (
                <button className='muahang' onClick={() => AddToCart(product)} >Add To Cart</button>
              )}
      ...
      {isShowCart && <ShoppingCart setShowCart={setShowCart} Cart={Cart}  setCart={setCart} />}
    </div>
  )
}
