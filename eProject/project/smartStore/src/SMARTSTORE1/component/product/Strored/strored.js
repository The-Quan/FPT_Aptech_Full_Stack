import React from 'react';
import Header from '../../ues/header/Header';
import Menu from '../../ues/menu/menu';
import Menudacap from '../../ues/menudacap/menudacap1';
import anh1 from './img/anh22.jpg';
import anh2 from './img/anh23.jpg';
import anh3 from './img/anh24.jpg';
import anh4 from './img/anh25.jpg';
import anh5 from './img/anh26.jpg';
import anh6 from './img/anh28.jpg';
import anh7 from './img/anh29.jpg';
import anh8 from './img/anh30.jpg';
import Footer from '../../ues/footer/footer';
function Strored(){
    return(
           <div className='stroerd'>
        <Header />
        <Menu />
        <Menudacap/>
        <div className='prd-content4'>
         <h2 className='prd-h2'>Storage Products</h2>
        </div>
        <div className='prd-content-anh'>
        <div className='product-anh1'>
               <img className='imgproduct1' src={anh1}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
        <div className='product-anh1'>
               <img className='imgproduct1' src={anh2}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
            <div className='product-anh1'>
               <img className='imgproduct1' src={anh3}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
            <div className='product-anh1'>
               <img className='imgproduct1' src={anh4}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
            </div>
            <div className='prd-content-anh'>
        <div className='product-anh1'>
               <img className='imgproduct1' src={anh5}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
        <div className='product-anh1'>
               <img className='imgproduct1' src={anh6}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
            <div className='product-anh1'>
               <img className='imgproduct1' src={anh7}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
            <div className='product-anh1'>
               <img className='imgproduct1' src={anh8}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
            </div>
            <div className='prd-content-anh'>
        <div className='product-anh1'>
               <img className='imgproduct1' src={anh1}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
        <div className='product-anh1'>
               <img className='imgproduct1' src={anh2}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
            <div className='product-anh1'>
               <img className='imgproduct1' src={anh3}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
            <div className='product-anh1'>
               <img className='imgproduct1' src={anh4}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
            </div>
            <div className='prd-content-anh'>
        <div className='product-anh1'>
               <img className='imgproduct1' src={anh5}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
        <div className='product-anh1'>
               <img className='imgproduct1' src={anh6}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
            <div className='product-anh1'>
               <img className='imgproduct1' src={anh7}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
            <div className='product-anh1'>
               <img className='imgproduct1' src={anh8}></img><br />
               <span>Tủ quần áo gỗ MDF thiết kế sang trọng hiện đại GHS-51259</span><br />
               <strong>7.000$</strong>
            </div>
            </div>

            <Footer />
           </div>
    )
}
export default Strored;